...
# Icons shown in Polybar (будут отображаться шрифтом JetBrainsMono Nerd Font)
ICON_SMARTPHONE=''   # nf-fa-mobile
ICON_TABLET=''       # nf-fa-tablet
SEPERATOR='|'
...
show_devices (){
    ...
    for device in $(qdbus --literal org.kde.kdeconnect /modules/kdeconnect org.kde.kdeconnect.daemon.devices); do
        ...
        then
            # устройство онлайн и сопряжено
            battery="$(qdbus org.kde.kdeconnect "/modules/kdeconnect/devices/$deviceid/battery" org.kde.kdeconnect.device.battery.charge 2>/dev/null)"
            icon=$(get_icon "$battery" "$devicetype")
            devices+="%{A1:$DIR/kdeconnect.sh -n '$devicename' -i $deviceid -b $battery -m:}$icon%{A}$SEPERATOR"
        elif [ "$isreach" = "false" ] && [ "$ispaired" = "true" ]
        then
            # устройство сопряжено, но не в сети
            devices+="$(get_icon -1 "$devicetype")$SEPERATOR"
        else
            # устройство не сопряжено
            haspairing="$(qdbus org.kde.kdeconnect "/modules/kdeconnect/devices/$deviceid" org.kde.kdeconnect.device.hasPairingRequests 2>/dev/null)"
            if [ "$haspairing" = "true" ]
            then
...
get_icon () {
    ...
    case $1 in
    "-1")     color="$COLOR_DISCONNECTED" ;;
    "-2")     color="$COLOR_NEWDEVICE" ;;
    5*)     color="$COLOR_BATTERY_50" ;;
    6*)     color="$COLOR_BATTERY_60" ;;
    7*)     color="$COLOR_BATTERY_70" ;;
    8*)     color="$COLOR_BATTERY_80" ;;
    9*|100) color="$COLOR_BATTERY_90" ;;
    *)      color="$COLOR_BATTERY_LOW" ;;
    esac
    # Оборачиваем иконку в переключение на шрифт JetBrainsMono Nerd Font (font-0)
    echo "%{F$color}%{T0}$icon%{T-}%{F-}"
}
